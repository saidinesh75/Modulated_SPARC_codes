...
def eta_modulated_new(beta_hat,code_params,c,tau_t,delim,W_params):
    ...
    s1 = beta_hat
    Lr,Lc,Mr,Mc = map(W_params.get,['Lr','Lc','Mr','Mc'])
    ...
    for j in range(L):
        ...
        for k in range(K):
            new_exp_param = np.real(((beta_section.conj())*c[k]))/tau_t[int(j//(L/Lc))]  
